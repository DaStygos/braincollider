...
def create_demo_problem():
    """Create and print a demo problem with LaTeX formulas.

    This function is intentionally not executed on import so that test
    discovery (which imports modules) does not create persistent objects.
    Run the script directly (e.g. `python manage.py shell < test_latex.py`)
    and the demo will be created because `__name__ == '__main__'`.
    """
